...
fn hash_from_path_name(filename: &str) -> Option<&str> {
    // maybe just use regex, or not, because why
    let name = filename.split('.').next().unwrap();
    let hash = name.rsplit('-').next().unwrap();
    if hash.len() != 16 || hash.len() == name.len() {
        // the hash is the wrong length or we did not find a dash
        // it cant be a fingerprint matched file.
        return None;
    }
...
fn remove_not_built_with_in_a_profile(
    ...
    let mut total_disk_space = 0;
    total_disk_space += remove_not_matching_in_a_dir(&dir.join("build"), &keep, delete)?;
    total_disk_space += remove_not_matching_in_a_dir(&dir.join("deps"), &keep, delete)?;
    // examples is just final artifacts not tracked by fingerprint so skip that one.
    // incremental is not tracked by fingerprint so skip that one.
    // `native` isn't generated by cargo since 1.37.0
    let native_dir = dir.join("native");
    if native_dir.exists() {
        total_disk_space += remove_not_matching_in_a_dir(&native_dir, &keep, delete)?;
    }
    total_disk_space += remove_not_matching_in_a_dir(dir, &keep, delete)?;
    total_disk_space += remove_not_matching_in_a_dir(&dir.join(".fingerprint"), &keep, delete)?;
    Ok(total_disk_space)
}
...
pub fn remove_not_built_with(
    ...
    for fing in lookup_all_fingerprint_dirs(dir) {
        ...
        total_disk_space +=
            remove_not_built_with_in_a_profile(path.parent().unwrap(), &keep, delete)?;
    }
    Ok(total_disk_space)
}
...
pub fn remove_older_until_fits(path: &Path, target_size: u64, delete: bool) -> Result<u64, Error> {
    ...
    for (fing, keep) in organized {
        total_disk_space +=
            remove_not_built_with_in_a_profile(fing.parent().unwrap(), &keep, delete)?;
    }
